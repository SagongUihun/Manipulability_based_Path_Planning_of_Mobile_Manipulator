...
for k=-180:5:180
    ...
    for i=1:5
        randomguess = randomConfiguration(panda);
        config_q_TEMP(1,:) = ik('panda_link8',trvec2tform(MtoH_P(1,:))*axang2tform(MtoH_R3(1,:)),weights,randomguess);
...
%             disp("Violate joint limit!!")
        ...
        Jacob07_constraint = Jacob07(1,:);
        
        manipulability_best_of_5=sqrt(det(Jacob07_constraint * transpose(Jacob07_constraint)));
%         manipulability_best_of_5=sqrt(det(inv(Jacob07_constraint * transpose(Jacob07_constraint))));
...
for i=2:num_handle_P(1,1)
    ...
    for j=1:num_mobile_P(1,1)
        ...
        q(j,:) = ik('panda_link8',trvec2tform(MtoH_P(j,:))*axang2tform(MtoH_R3(1,:)),weights,homeguess);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%% joint limits check %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         fuck = 0;
        if  ws_error_flag==1
            ...
            Jacob07_constraint = Jacob07(1,:);
            manipulability_1(cnt,1)=sqrt(det(Jacob07_constraint * transpose(Jacob07_constraint)));
%             manipulability_1(cnt,1)=sqrt(det(inv(Jacob07_constraint * transpose(Jacob07_constraint))));
...
function next_P_candidate = next_P_candidate(now_P)
    door_width = 0.92;
    open_angle_step = 2.5;
    step = door_width * open_angle_step*pi/180; % 0.04m
    ang = 30;
