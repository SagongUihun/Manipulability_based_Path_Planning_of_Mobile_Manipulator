...
% mobile_P(1,:) = [-0.35, 0.92, 0.35];
mobile_P(1,:) = [-0.4, 0.92, 0.35];
...
Jacob07 = get_jacobian(config_q(1,1), config_q(1,2), config_q(1,3), config_q(1,4), config_q(1,5), config_q(1,6), config_q(1,7));
Jacob07_constraint = Jacob07(3,:);
manipulability_init=sqrt(det(Jacob07_constraint * transpose(Jacob07_constraint)));
% manipulability_init=sqrt(det(Jacob07 * transpose(Jacob07)));
selected_manipulability(1,:) = manipulability_init;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function next_P_candidate = next_P_candidate(now_P)
    ...
    for k=1:num_candidate
        next_P_candidate(k,:) = [now_P(1,1) + step*cosd((k)*ang), now_P(1,2) + step*sind((k)*ang), now_P(1,3)];
    end
