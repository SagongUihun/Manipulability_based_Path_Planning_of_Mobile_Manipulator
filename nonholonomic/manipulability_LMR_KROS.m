...
MtoH_P(1,1) = Px * cos(mobile_P(1,4)*pi/180) +  Py * sin(mobile_P(1,4)*pi/180)  ;
MtoH_P(1,2) = Py * cos(mobile_P(1,4)*pi/180) -  Px * sin(mobile_P(1,4)*pi/180)  ;
MtoH_P(1,3) = Pz  ;

MtoH_R1 = [0, 1, 0, pi/2];
MtoH_R2 = [0, 0, 1, -pi/4];
MtoH_R3 = [0, 0, 1, ( handle_P(1,4) - mobile_P(1,4) ) *pi/180];

POSE = trvec2tform(MtoH_P(1,:))*axang2tform(MtoH_R3(1,:))*axang2tform(MtoH_R1(1,:))*axang2tform(MtoH_R2(1,:));
config_q(1,:) = ik('panda_link8',POSE,weights,randomguess); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=2:num_handle_P(1,1)
    ...
    for j=1:num_mobile_P(1,1)
        ...
        MtoH_P(j,1) = Px * cos(mobile_next_Pose(j,4)*pi/180) + Py * sin(mobile_next_Pose(j,4)*pi/180);
        MtoH_P(j,2) = Py * cos(mobile_next_Pose(j,4)*pi/180) - Px * sin(mobile_next_Pose(j,4)*pi/180);
        MtoH_P(j,3) = Pz;
        ...
        POSE = trvec2tform(MtoH_P(j,:))*axang2tform(MtoH_R3(1,:))*axang2tform(MtoH_R1(1,:))*axang2tform(MtoH_R2(1,:));
        q(j,:) = ik('panda_link8',POSE,weights,randomguess); 
        ...
        if -0.0174 > q(j,6) ||  q(j,6)  > 3.7524
            jl_error_flag = 1;
        end

        if q(j,7) > 2.8972 || q(j,7) < -2.8972
            jl_error_flag = 1;
        end
        ...
        if  ws_error_flag==1 
            ...
            vec_A = [real_MtoH_P(cnt,1), real_MtoH_P(cnt,2)];
            vec_B = [handle_P(i,1), handle_P(i,2)];
            
            % 안전거리 0.3m, mobile이 x방향으로 문을 앞서지 않게
            if abs(norm(vec_A)) <= 0.3 || handle_P(i,1) - mobile_next_Pose(j,1) < 0 
    ...
    randomguess = real_q(max_manipul_cnt_1, :);
...
%     next_P_candidate(1,:) = [now_P(1,1)       , now_P(1,2)      , now_P(1,3)];
%     
%     next_P_candidate(2,:) = [now_P(1,1) + 0.05, now_P(1,2)      , now_P(1,3)];
%     next_P_candidate(3,:) = [now_P(1,1) + 0.10, now_P(1,2)      , now_P(1,3)];
%     next_P_candidate(4,:) = [now_P(1,1) + 0.15, now_P(1,2)      , now_P(1,3)];
%     
%     next_P_candidate(5,:) = [now_P(1,1) - 0.05, now_P(1,2)      , now_P(1,3)];
%     next_P_candidate(6,:) = [now_P(1,1) - 0.10, now_P(1,2)      , now_P(1,3)];
%     next_P_candidate(7,:) = [now_P(1,1) - 0.15, now_P(1,2)      , now_P(1,3)];
    front_step = 0.15;
    rotation_step = front_step / 0.2915 * 180 / pi;
    
    side_step_x = 0.2915 *(1- cos(rotation_step * pi /180));
    side_step_y = 0.2915 * sin(rotation_step * pi /180);
    

    rotation_step_L = now_P(1,4) + rotation_step;
    rotation_step_M = now_P(1,4) + 0;
    rotation_step_R = now_P(1,4) - rotation_step;
    
    next_P_candidate(1,:) = [now_P(1,1)       , now_P(1,2)      , now_P(1,3), rotation_step_M ];

    
    next_P_candidate(2,:) = [now_P(1,1) - side_step_x * sin(rotation_step * pi/180) + side_step_y * cos(rotation_step * pi/180),     now_P(1,2) + side_step_x * cos(rotation_step * pi/180) + side_step_y * sin(rotation_step * pi/180)      , now_P(1,3)   , rotation_step_L ];
    next_P_candidate(3,:) = [now_P(1,1) + front_step * cos(rotation_step * pi/180),     now_P(1,2) + front_step * sin(rotation_step * pi/180)                  , now_P(1,3)    ,   rotation_step_M ];
    next_P_candidate(4,:) = [now_P(1,1) + side_step_x * sin(rotation_step * pi/180) + side_step_y * cos(rotation_step * pi/180),     now_P(1,2) - side_step_x * cos(rotation_step * pi/180) + side_step_y * sin(rotation_step * pi/180)      , now_P(1,3)    , rotation_step_R ];
    
    next_P_candidate(5,:) = [now_P(1,1) - side_step_x * sin(rotation_step * pi/180) - side_step_y * cos(rotation_step * pi/180),     now_P(1,2) + side_step_x * cos(rotation_step * pi/180) - side_step_y * sin(rotation_step * pi/180)      , now_P(1,3)   , rotation_step_R ];
    next_P_candidate(6,:) = [now_P(1,1) - front_step * cos(rotation_step * pi/180),     now_P(1,2) - front_step * sin(rotation_step * pi/180)                 , now_P(1,3)    ,   rotation_step_M  ];
    next_P_candidate(7,:) = [now_P(1,1) + side_step_x * sin(rotation_step * pi/180) - side_step_y * cos(rotation_step * pi/180),     now_P(1,2) - side_step_x * cos(rotation_step * pi/180) - side_step_y * sin(rotation_step * pi/180)      , now_P(1,3)   , rotation_step_L  ];
end
